...
4) **scrape_X_pdf.py:** *Very rough* Python code to scrape confirmed cases by residence directly from the Epi Reports in pdf, and testing and ICU data from daily reports. *Disclaimer: I'm clearly not a Python coder, so code might be inefficient, but it works well with recent reports*.
